...
__attribute__((section(".tcm:code"))) void launchAppCode(){
    ...
    //Turn off red led
    P1OUT &= 0xfe; // Set output to 1 (red LED)
    ...
    while (address + (count*2) < lastAddress){
        ...
        /****** CALL  *********/
            ...
            if(outcome != REJECTED){
                ...
                if(cfi){
                    ...
                    if((opCode & 0x0030) == 0x0030 /* immediate mode */){
        ...
        /***** TWO OPERANDS INSTRUCTIONS ****/
            ...
            if((opCode & 0x0030) == 0x0010 /* Indexed, Absolute, Symbolic */ ||
                (opCode & 0x0030) == 0x0030 /* Immediate (BUG: it was masked with 0x0f30!)*/
            ){
            ...
            if((opCode & 0xf08f) == 0x4000){
                /***** Check SRC mode ********/
                /* Immediate mode */
                if( (opCode & 0x0030) == 0x0030 || /* Bug it was masked with 0x0f30 */
                ...
                if(outcome != REJECTED){
                    ...
                    if(cfi){
                        ...
                        if( /* immediate mode */
                            (opCode & 0x0030) == 0x0030  /*Bug: it was masked with 0x0f30*/ ||
                            /* Constant generators */
                            (opCode & 0x0f30) == 0x0330 ||
                            (opCode & 0x0f30) == 0x0300 ||
                            (opCode & 0x0f30) == 0x0310 ||
